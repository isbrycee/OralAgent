...
class CephalometricImageCVMstatusClassification(BaseModel):
...
class CephalometricImageCVMstatusClassificationOutput(BaseModel):
...
class CephalometricImageCVMstatusClassificationTool(BaseTool):
    """Tool for performing image-level CVM status analysis of cephmetric images."""

    name: str = "cephalometric_image_cvm_status_classification"
    description: str = (
        "Classifies cephmetric images into 9 categories related to image-level abnormalities, including CVM-S1, CVM-S2, CVM-S3, CVM-S4, CVM-S5, CVM-S6."
        "Ensure the input cephmetric image is of high resolution and quality for accurate classification."
    )
    
    args_schema: Type[BaseModel] = CephalometricImageCVMstatusClassification
    ...
    def _load_model(self, checkpoint_path: str, num_classes: int):
        ...
        model = DinoV3Classifier(task_name="cephalometric_cvm_status_9class", num_classes=num_classes)
    ...
    def _run(self, 
            ...
            ) -> CephalometricImageCVMstatusClassificationOutput:
        try:
            ...
            output = CephalometricImageCVMstatusClassificationOutput(predicted_class=self.id2name[pred_class[0]], confidence=float(pred_conf[0]))
            # Save visualization
            viz_path = None
